...
Primary project navigation lives in the left project sidebar, ordered Traces, Logs, Metrics, Dashboards, and AI Eval when enabled, with Project settings separated at the bottom. Pinned dashboard shortcuts appear above primary navigation when returned by `Query.dashboards`, and the Dashboards entry may expand to show visible custom dashboards. The sidebar must not repeat the selected company/project summary; context selection belongs in the global topbar. Settings and company administration are not topbar tabs. Live trace receiving is a mode inside Traces, not a separate navigation entry.
...
- `Traces`, `Logs`, `Metrics`, `Dashboards`, and `AI Eval` require selected project context. Live trace receiving is a Traces mode and shares the same project context.
