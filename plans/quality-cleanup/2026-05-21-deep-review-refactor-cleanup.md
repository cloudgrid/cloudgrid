...
- AsyncAPI AI Chat conversation get/create/archive responses described `data` as a direct conversation while Go control-plane and BFF use `data.conversation`. Resolved in current contracts: response schemas use the wrapper shape.
- Named AsyncAPI response contracts for AI provider and AI Chat subjects were represented by generic Go response types. Resolved on 2026-05-28: named Go response types exist, control-plane handlers return them, and `contracts:check` guards their presence.
- Control-plane AI Chat not-found paths use trace-specific `ERR-004` / `TRACE_NOT_FOUND`. Current specs now document that existing contract; a future generic or AI Chat-specific not-found code requires an error-taxonomy migration before implementation.
- `contracts:check` validated only a selected error taxonomy subset. Resolved on 2026-05-28: it now parses every `errors.yaml` entry and requires matching runtime problem and backend bridge-schema literals.
- Go aggregate coverage was 70.6% at inspection time. Resolved by Task 4 in this plan: final direct aggregate coverage reached 80.1%.
