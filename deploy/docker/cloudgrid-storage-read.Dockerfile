...
COPY go.work ./
COPY core core
RUN CGO_ENABLED=0 GOOS=linux go build -tags surrealdb -trimpath -ldflags="-s -w" -o /out/cloudgrid-storage-read ./core/storage-read/cmd/storage-read
...
COPY --from=builder --chown=${CLOUDGRID_IMAGE_UID}:${CLOUDGRID_IMAGE_GID} /out/cloudgrid-storage-read /cloudgrid-storage-read
USER ${CLOUDGRID_IMAGE_UID}:${CLOUDGRID_IMAGE_GID}
EXPOSE 8081
ENTRYPOINT ["/cloudgrid-storage-read"]
