...
COPY go.work ./
COPY core core
RUN CGO_ENABLED=0 GOOS=linux go build -trimpath -ldflags="-s -w" -o /out/cloudgrid-control-plane ./core/control-plane/cmd/control-plane

FROM ${CLOUDGRID_GO_RUNTIME_IMAGE} AS runtime
ARG CLOUDGRID_IMAGE_UID=10001
ARG CLOUDGRID_IMAGE_GID=10001
WORKDIR /
COPY --from=builder --chown=${CLOUDGRID_IMAGE_UID}:${CLOUDGRID_IMAGE_GID} /out/cloudgrid-control-plane /cloudgrid-control-plane
USER ${CLOUDGRID_IMAGE_UID}:${CLOUDGRID_IMAGE_GID}
EXPOSE 8084
ENTRYPOINT ["/cloudgrid-control-plane"]
