...
const meta = {
  "trace-overview": [
    "Traces",
    "Search history, watch live traffic, and pivot into evidence.",
    "cloudgrid · /traces · live evidence",
  ],
  "trace-detail": [
    "Trace detail",
    "Follow a request path, select a span, and inspect exact evidence.",
    "cloudgrid · /traces/trc_92ad6f",
  ],
  logs: [
    "Logs",
    "Search project logs and pivot to trace or span context.",
    "cloudgrid · /logs · service:payment-svc",
  ],
  metrics: [
    "Metrics",
    "Explore descriptors, query series, and open exemplar traces.",
    "cloudgrid · /metrics · http.server.duration",
  ],
  dashboards: [
    "Dashboards",
    "Saved operational boards from typed telemetry widgets.",
    "cloudgrid · /dashboards · checkout overview",
  ],
  "dataset-overview": [
    "Datasets",
    "Curate trace-backed examples for evaluation.",
    "cloudgrid · /ai-eval · datasets",
  ],
  "dataset-detail": [
    "Dataset detail",
    "Maintain rows, health, splits, and evaluation readiness.",
    "cloudgrid · /ai-eval/datasets/support-intents",
  ],
  evaluations: [
    "Evaluations",
    "Manage definitions, targets, run policy, and latest results.",
    "cloudgrid · /ai-eval · evaluations",
  ],
  "evaluation-detail": [
    "Evaluation result",
    "Inspect run metrics, item results, trace evidence, and next actions.",
    "cloudgrid · /ai-eval · evaluation run",
  ],
};
...
function traceOverview() {
  const rows = [
    [
      service("checkout-api", "#5865f2"),
      "POST /checkout",
      mono("trc_92ad6f"),
      "842 ms" + bar(92, "var(--red)"),
      chip("error", "err"),
      "31",
      "3",
    ],
    [
      service("agent-svc", "#8b5cf6"),
      "agent.run",
      mono("trc_a18fc2"),
      "611 ms" + bar(64),
      chip("ok", "ok"),
    ...
    [
      service("payments", "#f59e0b"),
      "POST /capture",
      mono("trc_c44391"),
      "1.22 s" + bar(100, "var(--red)"),
      chip("error", "err"),
      "24",
      "2",
    ],
    [
      service("frontend", "#10b981"),
      "GET /cart",
      mono("trc_d091be"),
      "122 ms" + bar(24),
      chip("ok", "ok"),
    ...
    [
      service("llm-router", "#38bdf8"),
      "llm.chat",
      mono("trc_b82190"),
      "2.80 s" + bar(78, "var(--amber)"),
      chip("warn", "warn"),
      "15",
      "1",
    ],
...
function facets() {
  ...
  return panel(
    "Facets",
    "server suggestions",
    `<div class="facet-body">${groups
      .map(
        ([name, values]) =>
          `<div class="facet-group"><div class="tiny-label">${name}</div>${values
            .map(
              ([value, count], i) =>
                `<div class="facet-row ${i === 0 ? "selected" : ""}"><span>${value}</span><span>${count}</span></div>`,
            )
            .join("")}</div>`,
      )
      .join("")}</div>`,
...
function traceDetail() {
  ...
  return `<div class="grid detail-grid">
    ...
    ${panel(
      "Span inspector",
      "payment.capture · client · 382 ms",
      `<div class="toolbar-tabs"><span class="tab active">Attributes</span><span class="tab">Events</span><span class="tab">Exceptions</span><span class="tab">Links</span></div><div class="inspector-body">
      ${[
        "http.method|POST",
...
function logs() {
  return `<div class="grid logs-grid">
    ...
    ${panel(
      ...
      "virtualized rows",
      table(
        ["Time", "Sev", "Service", "Trace / Span", "Message"],
        [
          [
            mono("12:04:18.890"),
            chip("ERROR", "err"),
            mono("payment-svc"),
            mono("trc_92ad / spn_41c"),
...
function metrics() {
  const metricRows = [
    ...
    ["gen_ai.token.count", "sum · tokens"],
    ["process.cpu.usage", "gauge · ratio"],
    ["db.client.duration", "histogram · ms"],
    ["queue.depth", "gauge · items"],
  ];
  return `<div class="grid metrics-grid">
    ...
    ${panel(
      ...
      "p95 · 1m · group by service",
      `${lineChart()}${table(
        ["Series", "Latest", "Points"],
        [
          [mono("checkout-api"), "842 ms", "240"],
          [mono("agent-svc"), "611 ms", "240"],
          [mono("payment-svc"), "1.22 s", "240"],
        ],
      )}`,
    )}
    ${panel(
      ...
      `<div class="toolbar-tabs"><span class="tab active">Descriptor</span><span class="tab">Attributes</span><span class="tab">Exemplars</span></div><div class="inspector-body">
      ${[
        "kind|histogram",
        "unit|ms",
        "temporality|delta",
...
function datasetOverview() {
  return `<div class="grid dataset-grid">
    ...
    ${panel(
      ...
      table(
        ["Name", "Family", "Version", "Ready rows", "Health", "Updated"],
        [
          [
            "support-intents",
            "classification",
...
function datasetDetail() {
  return `<div class="grid dataset-grid">
    ...
    ${panel(
      "Rows",
      "cursor-paginated storage-read data",
      table(
        ["Split", "Status", "Input preview", "Expected", "Reason", "Source"],
        [
          [
            chip("validation"),
            chip("ready", "ok"),
...
function evaluations() {
  return `<div class="grid eval-grid">
    ...
    ${panel(
      ...
      "dataset · target · latest run",
      table(
        ["Name", "Dataset", "Split", "Target", "Last run", "Primary metric"],
        [
          [
...
function evaluationDetail() {
  return `<div class="grid eval-grid">
    ...
    ${panel(
      ...
      table(
        ["Row", "Input", "Expected", "Actual", "Metric", "Trace"],
        [
          [
            mono("cls-041"),
            "refund request",
    ...
    ${panel(
      "Result inspector",
      "metric deltas · problems · actions",
      `<div class="inspector-body">${[
        "exact_match|82% +14pp",
        "pass_rate|91% +9pp",
        "groundedness|88% +6pp",
        "latency_p95|420ms -18%",
      ]
        .map((v) => {
          const [k, val] = v.split("|");
          return `<div class="kv"><span>${k}</span><strong>${val}</strong></div>`;
        ...
        .join(
          "",
        )}<div class="notice">3 validation rows regressed on billing terms. Open failed rows before promotion.</div></div>`,
    )}
  </div>`;
}
...
function renderNav(active) {
  navTarget.innerHTML = nav
    .map(
      ([id, label, count]) =>
        `<button class="nav-item ${id === active ? "active" : ""}" data-screen="${id}"><span>${label}</span><span class="count">${count}</span></button>`,
    )
    .join("");
...
function render() {
  ...
  actionTarget.innerHTML = (actions[active] ?? [])
    .map((label, i) => `<button class="button ${i === 0 ? "primary" : ""}">${label}</button>`)
    .join("");
  renderNav(active);
