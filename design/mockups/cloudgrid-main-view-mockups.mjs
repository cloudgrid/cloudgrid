...
const traceOverview = () => {
  ...
  const tableY = y + 92;
  return svg("CloudGrid trace overview mockup", shell("Traces", `
...
const traceDetail = () => {
  ...
  return svg("CloudGrid trace detail mockup", shell("Traces", `
...
const metrics = () => {
  ...
  const points = [72,66,60,70,82,78,88,76,92,98,90,104,112,106,118,130,122,138,126,144,150,142,156,164,152];
  const chartStep = (inspX - midX - 150) / (points.length - 1);
  const path = points.map((p,i)=>`${i?"L":"M"} ${midX+54+i*chartStep} ${y+342-p}`).join(" ");
  return svg("CloudGrid metrics mockup", shell("Metrics", `
...
const logs = () => {
  ...
  const inspX = W - 404;
  const rows = [
    ["12:04:18.890","ERROR","payment-svc","trc_92ad / spn_41c","provider declined capture request"],
    ["12:04:18.711","WARN","tool-router","trc_92ad / spn_22f","web_search rate-limit retry attempt=2"],
    ["12:04:18.532","INFO","llm-router","trc_a18f / spn_09a","openai.chat completion 832 tokens"],
  ...
  return svg("CloudGrid logs mockup", shell("Logs", `
...
const datasetOverview = () => {
  ...
  return svg("CloudGrid dataset overview mockup", shell("Evaluations", `
...
const datasetDetail = () => {
  const x = routeX + pad;
  const y = routeY + 24;
  return svg("CloudGrid dataset detail mockup", shell("Evaluations", `
    ...
    ${["Ready rows|1,240","Schema health|ready","Train/val/test|70 / 20 / 10","Last import|12m ago"].map((m,i)=>{const [a,b]=m.split("|"); return `${t(x+24+i*280,y+126,a,"label")}${t(x+24+i*280,y+154,b,"title")}`}).join("")}
    ...
    ${[
      ["validation","ready","refund request after duplicate charge","billing_refund","explicit refund wording","trace trc_92ad","12m ago"],
      ["train","ready","cancel subscription and ask for VAT","cancellation","mixed billing terms","manual","1h ago"],
      ["test","draft","where is order ORD-10482","order_status","needs expected confirmation","csv import","2h ago"],
      ["validation","ready","agent gave wrong shipping answer","shipping_policy","trace-derived failure","trace trc_c443","3h ago"],
    ].map((r,i)=>{const ry=y+299+i*66; return `${line(x,ry+66,W-pad,ry+66,c.line2)}${chip(x+18,ry+20,r[0],"neutral",78)}${chip(x+110,ry+20,r[1],r[1]==="ready"?"ok":"warn",70)}${t(x+220,ry+38,r[2],"body")}${t(x+448,ry+38,r[3],"mono")}${t(x+668,ry+38,r[4],"muted")}${t(x+860,ry+38,r[5],"mono-muted")}${t(x+998,ry+38,r[6],"muted")}`}).join("")}
  `));
};
...
const evaluationOverview = () => {
  const x = routeX + pad;
  const y = routeY + 96;
  return svg("CloudGrid evaluations overview mockup", shell("Evaluations", `
    ...
    ${[
      ["support-quality-validation","support-intents","validation","agent-v4","complete","pass rate 91%","12m ago"],
      ["order-json-regression","order-extraction","test","extractor-v2","running","exact match 82%","now"],
      ["refund-policy-grounding","refund-policy","validation","rag-v3","failed","grounded 74%","1h ago"],
      ["tool-routing-smoke","tool-routing","test","agent-v4","queued","action match 88%","4h ago"],
    ].map((r,i)=>{const ry=y+127+i*70; const tone=r[4]==="complete"?"ok":r[4]==="running"?"info":r[4]==="failed"?"err":"neutral"; return `${line(x,ry+70,W-pad,ry+70,c.line2)}${t(x+18,ry+40,r[0],"h")}${t(x+248,ry+40,r[1],"body")}${chip(x+446,ry+22,r[2],"neutral",82)}${t(x+544,ry+40,r[3],"mono")}${chip(x+720,ry+22,r[4],tone,86)}${t(x+858,ry+40,r[5],"mono-muted")}${t(x+1030,ry+40,r[6],"muted")}${button(W-156,ry+20,"Open","secondary",72)}`}).join("")}
  `));
};
...
const evaluationDetail = () => {
  ...
  const y = routeY + 24;
  const inspectorX = W - 404;
  return svg("CloudGrid evaluation detail result mockup", shell("Evaluations", `
    ...
    ${["Status|complete","Pass rate|91%","Exact match|82%","p95 latency|420 ms","Regressions|3"].map((m,i)=>{const [a,b]=m.split("|"); return `${t(x+24+i*158,y+126,a,"label")}${t(x+24+i*158,y+158,b,"title")}${i>0? t(x+24+i*158,y+184,["+9pp","+14pp","-18%","-5"][i-1],"mono",`fill="${i===4?c.amber:c.green}"`):""}`}).join("")}
    ...
    ${[
      ...
      ["cls-077","cancellation","billing_invoice","fail","VAT detail outweighed intent","trc_c443"],
      ["ext-022","FR","FR","pass","normalized country value","trc_d091"],
    ].map((r,i)=>{const ry=y+359+i*66; return `${line(x,ry+66,inspectorX-12,ry+66,c.line2)}${t(x+18,ry+38,r[0],"mono")}${t(x+122,ry+38,r[1],"mono")}${t(x+268,ry+38,r[2],"mono")}${chip(x+402,ry+20,r[3],r[3]==="pass"?"ok":"err",62)}${t(x+518,ry+38,r[4],"muted")}${t(x+840,ry+38,r[5],"mono",`fill="${c.blue}"`)}`}).join("")}
