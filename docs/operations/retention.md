...
Saved policies are durable configuration. The repository includes the storage-maintenance batch-executor module and service image/chart shape, but production deletion still depends on wiring the executor to the intended storage adapter and scheduler. Retention audit execution remains follow-on work.
...
Review the concept page: [Retention and alerts](../concepts/retention-alerts.md).
