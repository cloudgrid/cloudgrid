...
| IR-010 | partial | AI Eval v2 product, backend, frontend, NFR, GraphQL, AsyncAPI, entity schema, generated TypeScript, generated Go, BFF foundation, storage-write persistence, storage-read query semantics, runner orchestration, frontend workspace, integration fixtures, docs, and runtime v2 dataset shape normalization are aligned. `plans/ai-eval-v2-migration` is implementation-ready, `TICKET-201` through `TICKET-205` are complete, `TICKET-206` is unblocked/in progress, and `bun run contracts:check`, `bun run typecheck`, focused BFF/frontend/integration tests, website build, spec check, plan lint, skills check, and wave readiness checks pass. | Complete `TICKET-206` final verification by rerunning Go workspace tests and browser/API end-to-end acceptance in an environment with Go and browser automation available. |
