...
| IR-010 | partial | AI Eval v2 product, backend, frontend, NFR, GraphQL, AsyncAPI, entity schema, generated TypeScript, generated Go, BFF foundation, storage-write persistence, storage-read query semantics, runner orchestration, and frontend workspace are aligned. `plans/ai-eval-v2-migration` is implementation-ready, `TICKET-201` through `TICKET-205` are complete, and `bun run contracts:check`, `bun run typecheck`, focused frontend/storage/runner tests, browser shell checks, spec check, plan lint, and wave readiness checks pass. | Complete `TICKET-206`: integration fixtures, docs, final verification gates, and runtime v2 data-shape drift discovered by end-to-end checks. |
