...
func (store *testStore) GetUser(_ context.Context, userID string) (ports.UserRecord, bool, error) {
	store.mu.RLock()
...
func (store *testStore) PutUser(_ context.Context, user ports.UserRecord) error {
	store.mu.Lock()
...
func (store *testStore) GetOrganization(_ context.Context, organizationID string) (ports.OrganizationRecord, bool, error) {
	store.mu.RLock()
	defer store.mu.RUnlock()
...
func (store *testStore) PutOrganization(_ context.Context, organization ports.OrganizationRecord) error {
	store.mu.Lock()
	defer store.mu.Unlock()
...
func (store *testStore) ListOrganizations(_ context.Context) ([]ports.OrganizationRecord, error) {
	store.mu.RLock()
	defer store.mu.RUnlock()
...
func (store *testStore) GetMembership(_ context.Context, organizationID string, userID string) (ports.MembershipRecord, bool, error) {
	store.mu.RLock()
	defer store.mu.RUnlock()
...
func (store *testStore) PutMembership(_ context.Context, membership ports.MembershipRecord) error {
	store.mu.Lock()
	defer store.mu.Unlock()
...
func (store *testStore) DeleteMembership(_ context.Context, organizationID string, userID string) error {
	store.mu.Lock()
	defer store.mu.Unlock()
...
func (store *testStore) ListMemberships(_ context.Context, organizationID string) ([]ports.MembershipRecord, error) {
	store.mu.RLock()
	defer store.mu.RUnlock()
...
func (store *testStore) ListMembershipsForUser(_ context.Context, userID string) ([]ports.MembershipRecord, error) {
	store.mu.RLock()
	defer store.mu.RUnlock()
	items := []ports.MembershipRecord{}
	for _, membership := range store.memberships {
...
func (store *testStore) GetInvitation(_ context.Context, invitationID string) (ports.InvitationRecord, bool, error) {
	store.mu.RLock()
	defer store.mu.RUnlock()
...
func (store *testStore) ListInvitations(_ context.Context, organizationID string) ([]ports.InvitationRecord, error) {
	store.mu.RLock()
	defer store.mu.RUnlock()
...
func (store *testStore) GetPendingInvitationByEmail(_ context.Context, organizationID string, email string) (ports.InvitationRecord, bool, error) {
	store.mu.RLock()
	defer store.mu.RUnlock()
	for _, invitation := range store.invitations {
		if invitation.OrganizationID == organizationID &&
...
func (store *testStore) PutProject(_ context.Context, project ports.ProjectRecord) error {
	store.mu.Lock()
	defer store.mu.Unlock()
...
func (store *testStore) ListProjects(_ context.Context, organizationID *string, status *contracts.ProjectStatus) ([]ports.ProjectRecord, error) {
	store.mu.RLock()
	defer store.mu.RUnlock()
...
func (store *testStore) GetIngestCredential(_ context.Context, credentialID string) (ports.IngestCredentialRecord, bool, error) {
	store.mu.RLock()
	defer store.mu.RUnlock()
...
func (store *testStore) PutIngestCredential(_ context.Context, credential ports.IngestCredentialRecord) error {
	store.mu.Lock()
	defer store.mu.Unlock()
...
func (store *testStore) ListIngestCredentials(_ context.Context, projectID string) ([]ports.IngestCredentialRecord, error) {
	store.mu.RLock()
	defer store.mu.RUnlock()
...
func (store *testStore) GetDashboard(_ context.Context, dashboardID string) (ports.DashboardRecord, bool, error) {
	store.mu.RLock()
	defer store.mu.RUnlock()
...
func (store *testStore) PutDashboard(_ context.Context, dashboard ports.DashboardRecord) error {
	store.mu.Lock()
	defer store.mu.Unlock()
...
func (store *testStore) DeleteDashboard(_ context.Context, dashboardID string) error {
	store.mu.Lock()
	defer store.mu.Unlock()
...
func (store *testStore) ListDashboards(_ context.Context, projectID string) ([]ports.DashboardRecord, error) {
	store.mu.RLock()
	defer store.mu.RUnlock()
...
func (store *testStore) ListDashboardPins(_ context.Context, userID string, projectID string) ([]ports.DashboardPinRecord, error) {
	store.mu.RLock()
	defer store.mu.RUnlock()
...
func (store *testStore) PutDashboardPin(_ context.Context, pin ports.DashboardPinRecord) error {
	store.mu.Lock()
	defer store.mu.Unlock()
...
func (store *testStore) DeleteDashboardPin(_ context.Context, userID string, projectID string, dashboardID string) error {
	store.mu.Lock()
	defer store.mu.Unlock()
...
func (store *testStore) DeleteDashboardPinsForDashboard(_ context.Context, dashboardID string) error {
	store.mu.Lock()
	defer store.mu.Unlock()
	for key, pin := range store.dashboardPins {
		if pin.DashboardID == dashboardID {
...
func (store *testStore) GetProjectMember(_ context.Context, projectID string, userID string) (ports.ProjectMemberRecord, bool, error) {
	store.mu.RLock()
	defer store.mu.RUnlock()
...
func (store *testStore) PutProjectMember(_ context.Context, member ports.ProjectMemberRecord) error {
	store.mu.Lock()
	defer store.mu.Unlock()
...
func (store *testStore) DeleteProjectMember(_ context.Context, projectID string, userID string) error {
	store.mu.Lock()
	defer store.mu.Unlock()
...
func (store *testStore) DeleteProjectMembershipsForUserInOrganization(_ context.Context, organizationID string, userID string) error {
	store.mu.Lock()
	defer store.mu.Unlock()
...
func (store *testStore) ListProjectMembers(_ context.Context, projectID string) ([]ports.ProjectMemberRecord, error) {
	store.mu.RLock()
	defer store.mu.RUnlock()
...
func (store *testStore) GetRetentionPolicy(_ context.Context, projectID string) (ports.RetentionPolicyRecord, bool, error) {
	store.mu.RLock()
	defer store.mu.RUnlock()
...
func (store *testStore) PutRetentionPolicy(_ context.Context, policy ports.RetentionPolicyRecord) error {
	store.mu.Lock()
	defer store.mu.Unlock()
	policy.Rules = append([]ports.RetentionRuleRecord{}, policy.Rules...)
...
func (store *testStore) GetProjectAiSettings(_ context.Context, projectID string) (ports.ProjectAiSettingsRecord, bool, error) {
	store.mu.RLock()
	defer store.mu.RUnlock()
	settings, ok := store.projectAiSettings[projectID]
	settings.Settings = cloneMap(settings.Settings)
	return settings, ok, nil
}

func (store *testStore) PutProjectAiSettings(_ context.Context, settings ports.ProjectAiSettingsRecord) error {
	store.mu.Lock()
	defer store.mu.Unlock()
	settings.Settings = cloneMap(settings.Settings)
...
func (store *testStore) GetAlertRule(_ context.Context, id string) (ports.AlertRuleRecord, bool, error) {
	store.mu.RLock()
	defer store.mu.RUnlock()
...
func (store *testStore) PutAlertRule(_ context.Context, rule ports.AlertRuleRecord) error {
	store.mu.Lock()
	defer store.mu.Unlock()
...
func (store *testStore) DeleteAlertRule(_ context.Context, id string) error {
	store.mu.Lock()
	defer store.mu.Unlock()
...
func (store *testStore) ListAlertRules(_ context.Context, projectID string) ([]ports.AlertRuleRecord, error) {
	store.mu.RLock()
	defer store.mu.RUnlock()
...
func (store *testStore) GetAlertSilence(_ context.Context, id string) (ports.AlertSilenceRecord, bool, error) {
	store.mu.RLock()
	defer store.mu.RUnlock()
...
func (store *testStore) PutAlertSilence(_ context.Context, silence ports.AlertSilenceRecord) error {
	store.mu.Lock()
	defer store.mu.Unlock()
...
func (store *testStore) DeleteAlertSilence(_ context.Context, id string) error {
	store.mu.Lock()
	defer store.mu.Unlock()
...
func (store *testStore) ListAlertSilences(_ context.Context, projectID string, ruleID *string) ([]ports.AlertSilenceRecord, error) {
	store.mu.RLock()
	defer store.mu.RUnlock()
...
func (store *testStore) PutAlertEvent(_ context.Context, event ports.AlertEventRecord) error {
	store.mu.Lock()
	defer store.mu.Unlock()
...
func (store *testStore) ListAlertEvents(_ context.Context, projectID string, ruleID *string, first int, after *string) ([]ports.AlertEventRecord, bool, *string, error) {
	store.mu.RLock()
	defer store.mu.RUnlock()
