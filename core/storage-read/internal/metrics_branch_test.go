...
func TestMetricsRecorderBranches(t *testing.T) {
	...
	recorder.Increment("cloudgrid.live.subscriptions", 2, map[string]string{"operation": "live"})
	recorder.Observe("cloudgrid.storage.read.duration", 0.5, map[string]string{"operation": "trace"})
	if len(capture.events) != 2 {
		t.Fatalf("recorded events = %d, want 2", len(capture.events))
	}
	if capture.events[0].Kind != selfobs.MetricKindUpDownCounter || capture.events[1].Kind != selfobs.MetricKindHistogram {
		t.Fatalf("metric kinds = %s/%s", capture.events[0].Kind, capture.events[1].Kind)
	}

	memory := NewInMemoryMetricsRecorder()
	labels := map[string]string{"operation": "trace"}
	memory.Increment("requests", 1, labels)
	memory.Observe("duration", 0.25, labels)
	labels["operation"] = "mutated"
	snapshot := memory.Snapshot()
	if len(snapshot) != 2 || snapshot[0].Labels["operation"] != "trace" {
		t.Fatalf("snapshot = %#v", snapshot)
	}
	snapshot[0].Labels["operation"] = "changed"
	if again := memory.Snapshot(); again[0].Labels["operation"] != "trace" {
