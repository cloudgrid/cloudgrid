...
type readinessManagerOptions struct {
...
type readinessManager struct {
	...
	options    readinessManagerOptions
	initialize bool
}

func newReadinessManager(options readinessManagerOptions) *readinessManager {
	return &readinessManager{state: newRetryableState(), options: options}
}

func (manager *readinessManager) check(ctx context.Context) error {
	if manager.state.degraded() && manager.options.reconnect != nil {
		if err := manager.options.reconnect(ctx); err != nil {
			return manager.state.observeOperationError(err)
		}
		if manager.initialize && manager.options.initialize != nil {
			if err := manager.options.initialize(ctx); err != nil {
				return manager.state.observeOperationError(err)
			}
		}
	}
	if manager.options.readiness == nil {
		manager.state.markReady()
		return nil
	}
	if err := manager.options.readiness(ctx); err != nil {
		return manager.state.observeOperationError(err)
	}
	manager.state.markReady()
	return nil
