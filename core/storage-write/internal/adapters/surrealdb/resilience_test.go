...
func TestStorageWriteRetryableStateDegradesAndRecovers(t *testing.T) {
	state := newRetryableState()

	err := state.observeOperationError(errors.New("use of closed network connection"))
...
func TestStorageWriteSDKLockHonorsContextDeadline(t *testing.T) {
